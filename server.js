...
client.on("ready", () => {
  client.user.setActivity(`Factory | Uptime Site`);
  passport.use(
    new Strategy(
      {
...
linkss = linkA
 ...
 if(db.get("linkler").map(z => z.url).includes(link)) {
      ...
      res.send("Başarıyla Eklendi! Factory Uptime İyi Günler Diler! Discord: https://discord.gg/3Rh3RbPd İD:" + req.user.id);
    }
  });
